...
function J_φ_ABS_sch(ξ, φ)
    ...
    integral = coeff * quadgk(
        ε -> quadgk(
            λ -> λ / R_sch(ξ, ε, λ) * exp(-β * γ * (ε + v * sqrt(ε^2 - 1) * cos(φ) * cos(X_sch(ξ, ε, λ)))) *
                 sinh(β * γ * v * sqrt(ε^2 - 1) * cos(φ) * cos(X_sch(ξ, ε, λ))), 0, λ_c_sch(ε))[1], 1, infinity)[1]
    return integral
end
...
#############KERR################
X_kerr(ξ, ε, λ, α) = quadde(_ξ_ -> (λ + (α * ε) / (1 - 2 / _ξ_)) / (((α^2) / (1 - 2 / _ξ_) + _ξ_^2) * sqrt(Complex(ε^2 - (1 - 2 / _ξ_) * (1 + λ^2 / _ξ_^2) - (α^2 + 2 * ε * λ * α) / (_ξ_^2)))), ξ, Inf)[1]
...
S(ξ, ε, λ, α, ϵ_σ, ϵ_r, φ) = exp(-(ε + ϵ_σ * v * sqrt(ε^2 - 1) * sin(φ - ϵ_σ * ϵ_r * (π / 2 - X_kerr(ξ, ε, λ, α)))) * β *γ)
...
function __jt_integrals__(ξ, λ, ε, α, ϵ_σ, ϵ_r, φ)
    if R̃_kerr(ξ, ε, α, λ, ϵ_σ) == 0
        ...
        temp = ε * S(ξ, ε, λ, α, ϵ_σ, ϵ_r, φ) / sqrt(R̃_kerr(ξ, ε, α, λ, ϵ_σ))# / (M * ξ) # Tu powinna być suma 2 składników z  ϵ_σ=+1 i ϵ_σ=-1
        return temp
    end
end
